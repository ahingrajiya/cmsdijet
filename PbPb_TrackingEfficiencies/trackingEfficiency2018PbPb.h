#ifndef TRKEFF2018PBPB_H
#define TRKEFF2018PBPB_H
...
inline bool TrkEff2018PbPb::checkBounds(float pt, float eta, int hiBin)
{
  ...
  {
    if (!isQuiet)
      std::cout << "TrkEff2018PbPb: track outside |eta|<2.4, please apply this cut!  I am returning a correction factor of 0 for this track for now." << std::endl;
    return false;
  }
  ...
  {
    if (!isQuiet)
      std::cout << "TrkEff2018PbPb: hiBin is outside the range [0,199].  Please fix this issue!  I am returning a correction factor of 0 for this track for now." << std::endl;
    return false;
  }
  ...
  {
    if (!isQuiet)
      std::cout << "TrkEff2018PbPb: pT is outside the range [0,500] with pT = " << pt << ".  I am returning a correction factor of 0 for this track for now." << std::endl;
    return false;
  }
...
inline float TrkEff2018PbPb::getCorrection(float pt, float eta, int hiBin)
{
  ...
  {
    if (!isQuiet)
      std::cout << "TrkEff2018PbPb: Warning! Tracking efficiency is very low for this track (close to dividing by 0).  Returning correction factor of 0 for this track for now." << std::endl;
    return 0;
  }
}

inline float TrkEff2018PbPb::getEfficiency(float pt, float eta, int hiBin, bool passesCheck)
{
  if (!passesCheck)
  {
    if (!checkBounds(pt, eta, hiBin))
  ...
  if (mode.compare("pixel") == 0)
  {
    ...
    if (hiBin >= 0 && hiBin < 10)
      return effPix[0]->GetBinContent(effPix[0]->FindBin(eta, pt));
    if (hiBin >= 10 && hiBin < 20)
      return effPix[1]->GetBinContent(effPix[1]->FindBin(eta, pt));
    if (hiBin >= 20 && hiBin < 60)
      return effPix[2]->GetBinContent(effPix[2]->FindBin(eta, pt));
    if (hiBin >= 60 && hiBin < 100)
      return effPix[3]->GetBinContent(effPix[3]->FindBin(eta, pt));
    if (hiBin >= 100 && hiBin < 200)
      return effPix[4]->GetBinContent(effPix[4]->FindBin(eta, pt));
  }

  return 0;
}

inline float TrkEff2018PbPb::getFake(float pt, float eta, int hiBin, bool passesCheck)
{
  if (!passesCheck)
  {
    if (!checkBounds(pt, eta, hiBin))
...
inline TrkEff2018PbPb::TrkEff2018PbPb(std::string collectionName, std::string quality, bool isQuiet_, std::string filePath)
{
  isQuiet = isQuiet_;
  mode = collectionName;
  ...
  {
    if (!isQuiet)
      std::cout << "TrkEff2018PbPb class opening in general tracks mode!" << std::endl;

    if (quality.compare("Loose") == 0)
      trkEff = TFile::Open((filePath + "2018PbPb_Efficiency_GeneralTracks_highPt_Loose.root").c_str(), "open");
    else if (quality.compare("Tight") == 0)
      trkEff = TFile::Open((filePath + "2018PbPb_Efficiency_GeneralTracks_highPt_Tight.root").c_str(), "open");
    else
    ...
    if (quality.compare("Loose") == 0)
      trkFake = TFile::Open((filePath + "2018PbPb_Efficiency_GeneralTracks_MB_Loose.root").c_str(), "open");
    else if (quality.compare("Tight") == 0)
      trkFake = TFile::Open((filePath + "2018PbPb_Efficiency_GeneralTracks_MB_Tight.root").c_str(), "open");
    else
  ...
  {
    if (!isQuiet)
      std::cout << "TrkEff2018PbPb class opening in general tracks positive mode!" << std::endl;
  ...
  {
    if (!isQuiet)
      std::cout << "TrkEff2018PbPb class opening in general tracks negative mode!" << std::endl;
  ...
  {
    if (!isQuiet)
      std::cout << "TrkEff2018PbPb class opening in pixel tracks mode!" << std::endl;
...
inline TrkEff2018PbPb::~TrkEff2018PbPb()
{
  trkEff->Close();
  trkFake->Close();
}
