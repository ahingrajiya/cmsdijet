...
ClassImp(JetCorrector) ClassImp(SingleJetCorrector)
    ...
    //_________________
    JetCorrector::JetCorrector() : JEC{}, JetPT{-999.}, JetEta{-999.}, JetPhi{-999.}, JetArea{-999.}, Rho{-999.}
{
    /* Empty */
}

//________________
JetCorrector::JetCorrector(std::string File) : JEC{}, JetPT{-999.}, JetEta{-999.}, JetPhi{-999.}, JetArea{-999.}, Rho{-999.}
{
    Initialize(File);
}

//________________
JetCorrector::JetCorrector(std::vector<std::string> Files) : JEC{}, JetPT{-999.}, JetEta{-999.}, JetPhi{-999.}, JetArea{-999.}, Rho{-999.}
{
    Initialize(Files);
}
...
SingleJetCorrector::SingleJetCorrector() :
    Initialized{false}, IsFunction{false}, JetPT{-999.}, JetEta{-999.}, JetPhi{-999.}, JetArea{-999.}, Rho{-999.}, Formulas{}, Parameters{}, BinTypes{}, BinRanges{},
...
double SingleJetCorrector::GetCorrection()
{
    ...
    for (int iE = 0; iE < N; iE++)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            if (Value < DependencyRanges[iE][i * 2]) Value = DependencyRanges[iE][i * 2];
            if (Value > DependencyRanges[iE][i * 2 + 1]) Value = DependencyRanges[iE][i * 2 + 1];
            V[i] = Value;
        }
        ...
        {
            if (Dependencies[iE].size() == 1) Function = new TF1(Form("Function%d", iE), (Formulas[iE] + "+0*x").c_str());
            if (Dependencies[iE].size() == 2) Function = new TF2(Form("Function%d", iE), (Formulas[iE] + "+0*x+0*y").c_str());
            if (Dependencies[iE].size() == 3) Function = new TF3(Form("Function%d", iE), (Formulas[iE] + "+0*x+0*y+0*z").c_str());
            if (Dependencies[iE].size() == 4) Function = new TF3(Form("Function%d", iE), (Formulas[iE] + "+0*x+0*y+0*z").c_str());
        ...
        for (int i = 0; i < (int)Parameters[iE].size(); i++) Function->SetParameter(i, Parameters[iE][i]);
        if (Dependencies[iE].size() == 4) Function->SetParameter(Parameters[iE].size(), GetValue(Dependencies[iE][3]));
