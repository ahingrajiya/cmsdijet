...
if [ "$doBuild" -eq 1 ]; then
    ...
    cp -r "$EXEC_PATH/build/PbPbDijetAna" "executions_dir"
fi

if [ "$DataSet" -eq 1 ]; then
    echo "PYTHIA + HYDJET Datasset is selected"    
    cd ${EXEC_PATH}   
    sample_prefix="PYTHIA_HYDJET"
    input_files_list="${EXEC_PATH}/files_input/PbPb5020/PH/"
